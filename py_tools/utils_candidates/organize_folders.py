...
organize_folders.py
-------------------
This script organizes files in a directory by date and extension, handling duplicates by moving them to a separate folder.
To run the script, provide the path to the folder you want to organize when prompted.

The script performs the following steps:
1. Create a "Duplicates" folder to store duplicate files.
2. Iterate over all files in the directory.
3. Get the modification date of each file.
    a. Create a folder for the modification date if it doesn't exist.
    b. Calculate the SHA-256 hash of the file.
    c. Check for duplicates using the file hash.
        i. If the file is a duplicate, move it to the "Duplicates" folder.
        ii. If the file is not a duplicate, move it to the date folder.
        iii. Organize files within the date folder by extension.
4. Print messages for each file moved or duplicate found.
'''
